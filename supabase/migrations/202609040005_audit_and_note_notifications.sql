...
begin
  ...
  identity_row := coalesce(after_row, before_row, '{}'::jsonb);
  row_id := coalesce(
    nullif(identity_row ->> 'id', '')::uuid,
    nullif(identity_row ->> 'task_id', '')::uuid,
    nullif(identity_row ->> 'staff_id', '')::uuid,
    nullif(identity_row ->> 'warehouse_id', '')::uuid
  );
...
drop trigger if exists audit_staff_warehouses on public.staff_warehouses;
create trigger audit_staff_warehouses after insert or update or delete on public.staff_warehouses
for each row execute function public.record_audit_event();
drop trigger if exists audit_task_items on public.task_items;
create trigger audit_task_items after insert or update or delete on public.task_items
for each row execute function public.record_audit_event();
...
for each row execute function public.record_audit_event();
drop trigger if exists audit_task_notes on public.task_notes;
create trigger audit_task_notes after insert or update or delete on public.task_notes
for each row execute function public.record_audit_event();
